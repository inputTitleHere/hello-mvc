...
-- 페이지 쿼리 작성하기 (top-n분석)
-- 1. rownum
-- rownum을 새로 부여하려면 where 조건절 변경 / inline view를 사용해야 한다.
-- offset이 있는 경우 inline view를 한레벨 더 사용해야 한다.
select
    *
from(
    select 
...
-- 회원 검색 결과의 페이징 예상 sql
--  select * from (select row_number() over(order by enroll_date desc) rnum, m.* from member m where # like ?) m where rnum between ? and ?
...
m.* 
from member m where member_id like '%a%'
) m where rnum between 11 and 22;

-- 검색된 회원수 조회
-- select count(*) from member where member_id like ?
...
create table board(
    ...
    writer varchar2(20), 
    content varchar2(4000) not null, -- 진짜 큰 게시글은 long같은거 쓰자.
    read_count number default 0,
    reg_date date default sysdate,
    constraint pk_board_no primary key(no),
    constraint fk_board_writer foreign key(writer) references member(member_id) on delete set null -- 회원 탈퇴시
);
